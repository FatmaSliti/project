...
const Payment_Buttons: React.FC = () => {
  ...
  //           <label
  //             key={index}
  //             className={`btn btn-color-gray-600 btn-active-text-gray-800 w-100 px-4 py-2 ${
  //               selectedButton === index ? 'border-2 border-blue-500 btn-outline ' : 'border-1'
  //             }`}
  //             style={{
        ...
        >
          {ButtonsData.map((data: ButtonData, index: number) => (
            <label
              ...
              onClick={() => handleButtonClick(index)}
            >
              <input className='btn-check' type='radio' name='method' defaultValue={0} checked={selectedButton === index} />
              <img
